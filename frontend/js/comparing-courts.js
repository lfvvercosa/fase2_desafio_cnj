setGraphsComponent()
setCommentsComponent()
setTableComponent()

function setGraphsComponent() {
  //graph-container-left-court
  //graph-container-right-court
}

function setCommentsComponent() {
  var comments = 'winwifiwoe wefinwof fwwienfwinfio fwwienfiwenfwnfw wfeionweoifnwiofw efweiunfiwenf wfiuwenfinwe fwiofnweifne'
  $('#comments').html(comments) 
}

function setTableComponent() {
  //best movements table
  $('#table-best-tab1').html('Mandado-Recebimento')
  //table-best-tab2
  //table-best-tab3

  $('#table-best-row-1-name').html('Awesome Metal Keyboard')
  $('#table-best-row-1-duration').html('3 meses')
  $('#table-best-row-1-comments').html('Nihil officia fuga consequatur omnis. Animi qui quia quis vel perspiciatis neque velit. Corrupti omnis sint.')
  
  //table-best-row-2-name
  //table-best-row-2-duration
  //table-best-row-2-comments
  
  //table-best-row-3-name
  //table-best-row-3-duration
  //table-best-row-3-comments
  
  //table-best-row-4-name
  //table-best-row-4-duration
  //table-best-row-4-comments
  
  //table-best-row-5-name
  //table-best-row-5-duration
  //table-best-row-5-comments
  
  //table-best-row-6-name
  //table-best-row-6-duration
  //table-best-row-6-comments
  
  //table-best-row-7-name
  //table-best-row-7-duration
  //table-best-row-7-comments
  
  //table-best-row-8-name
  //table-best-row-8-duration
  //table-best-row-8-comments

  //table-best-row-9-name
  //table-best-row-9-duration
  //table-best-row-9-comments
}