...
function selectGroup(group) {
  ...
  getBestCourts(selectedGroup.identificador, (data) => {
    $("#rank_courts").html("")
    //best 
    $("#rank_courts").append('<tr class="table-title"><td colspan="9">Unidades jurídicas com os melhores tempos de conclução de processo</td></tr>');
    ...
    //worst
    $("#rank_courts").append('<tr class="table-title"><td colspan="9">Unidades jurídicas com os piores tempos de conclução de processo</td></tr>')
  ...
  courts.forEach(court => {
    if (!filter || filter == "" || (filter == court.tribunal)) {
      var row = "<tr><td>" + court.ranking + "</td><td>" + court.nome + "</td><th>+" + court.tribunal +
        ...
        + court.processos + "</td><td>" + court.melhorEtapa + "</td><td>" + court.piorEtapa + "</td><td><button class='modal-button modal-button-warning' type='button' data-toggle='tooltip' title='Enviar alerta para este tribunal'><span data-toggle='modal' data-target='#alertasSugeridos'><i class='fas fa-bullhorn'></i></span></button><button class='modal-button modal-button-award' type='button' data-toggle='tooltip' title='Enviar alerta para este tribunal'><span data-toggle='modal' data-target='#alertasSugeridos'><i class='fas fa-award'></i></span></button></td></tr>"
...
function fillCourtFilter() {
  $('#cboTribunal').html('<option select value="0">Todas os tribunais</option>')
  var courts = []
